...
| PM `0x06c6` clear hold (`Card._hold_tx_block()`, `tools/dial_tikrnl_drive.py`) | enabled | `0x026a` | Resident kernel zeroes `DM(0x3fa7..0x3fac)` every sample in the frame path; host NOPs the store while page 14 is resident and restores it on the way out | Session 267 census: the generator refills the six-word mapping frame 0.167/frame and the clear runs 1.000/frame, so exactly 16.7% of published samples were nonzero in every transmitting state; with the hold the figure is 100% and the transmit's zero-crossing rate and run lengths match the real card's capture | **Active intervention, the direct backend's copy of the native tower's PM `0x06cd` hold.** `EICON_V90D_TX_BLOCK_HOLD=0` restores the old behaviour (one sample in six). It patches firmware, so it stands in for an unrecovered frame-path cadence rather than explaining it.
