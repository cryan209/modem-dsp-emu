...
| Portable bulk-delay servicing (`_service_bulk_adapter()`, 4397+) | V90D/V.34 enabled by defaults | `0x026a`, `0x0261` | Host services the delay-line ABI and holds the native worker at RTS | Native worker corrupted shared state before its descriptor/rate ABI was coherent | **Disabled by sport mode.** It remains a compatibility intervention, not a SPORT test.
| PM `0x19c8` bulk-worker RTS patch (5115+) | enabled by V90D/V34 bulk settings | `0x026a`, `0x0261` | Firmware bulk worker is replaced with RTS | Same native-width/descriptor failures as portable service | **Disabled by sport mode.** Legacy A/B only.
