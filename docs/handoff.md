...
| **the V.34 page freezes** | **still open, but localised to one instruction pair.** It is a **hang, not a slow clock** — 114m's "200x slow" and 114o's "8x slow" are that hang averaged over page residency. PM `0x2e24` is the block loader's **field unpacker** (`ret=0x2ddb`): it scans a block record three words at a time, testing every third low byte against `MR1 = 0x0019`, and scatters fields into `0x2137 + field`. Eleven entries per call; the eleventh is block **`0x1afa`**, and it never returns — 7.8M iterations against 4,576 for the whole rest of the call, and not one sample dispatched afterwards. Ruled out along the way: the callback `DM(0x3FB3)` is never rewritten (two writes all call, all 568 V.34-era dispatches correct); the loop is ordinary machinery that runs bounded on V.90 and INFO; and the stride is `M5 = 1` with `L4 = 0`, linear. By elimination the record at `0x1afa` is not what the overlay image says by the time it is scanned — same class as 114k, different address, hang rather than stray words. Next: `--watch-dm 0x1afa,0x1afd,0x1b00`, the exact offsets the scan tests | Sessions 76–79, 114b–o, **114p–s** |
