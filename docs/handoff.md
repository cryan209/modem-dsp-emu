...
| **V.90 loopback: the `0x0060` park is fixed; the answerer now matches `run48` everywhere the control reaches** | `EICON_EXPAND_SPORT=1` on the direct PRI backend. The answerer walks `0060 → 0062 → 0064 → 0066 → 0068 → 006a → 0070 → 0072 → 0074 → 0076 → 0078 → 007a → 007b → 0080 → 00b0` — run48's own walk — with no `0x5678` and no fallback, and its transmit on the wire (peak 988, 3 codepoints) matches run48's (peak 924, 2 codepoints) for the same phase. `run48`'s capture ends at `0x00b0`, so the answering side is done as far as any control can show. **The remaining blocker is the caller at `0x0092`**, waiting on bit 11 of `DM(0x20EF)`, which takes exactly one write per call — `0000`, from the record unpacker. Now fully characterised: bit 11 is the twelfth entry of an action-vector table (`DM(0x0088)` → `PM 0x30B2`, `RXD0`/`RXD1` = `0xFFFF`), no image in the analog109 set can write the word and no record in the 55-record table sets it, and a state-gated pin that skips the park makes the **answerer** fall back to INFO on 3/3 runs — so the park is real and the bit has to come from the host. **Generalised and acted on (251): bit 11 is not one missing bit, it is the first of a status *vocabulary*.** Every wait state on the V.90A page polls a bit of the same two words, through the handler table at `DM(0x064B)`: `0x0092` → `PM 0x3492`, bit 11 of `DM(0x20EF)`; `0x0095` → `PM 0x348F`, bit 14 of `DM(0x20EB)`; `0x00b3` → `PM 0x34C6`, bit 12 of `DM(0x20EF)`; `0x00c0` → `PM 0x3495`, bit 15 of `DM(0x20EB)` **and** `DM(0x10D9)` non-zero; `0x00c3` → `PM 0x349D`, bit 12 of `DM(0x20EB)`. None of those bits is set by any of the 55 records (the table is the only one — a full scan of `DM 0x1000..0x2000` finds exactly one chain, and the 23-entry next-address table at `DM(0x06B0)` never leaves it), and an absolute-store scan of every `pm.bin` in the analog109 set finds no writer for either word other than V29FC's unrelated reuse. So the whole vocabulary comes from outside the DSP, and supplying it walks the pairing a very long way: with `EICON_ANALOG_PIN_DM=0x20ef=0x0800@0x20f9:0x0092>16,0x20eb=0x4000@0x20f9:0x0095,0x20ef=0x1000@0x20f9:0x00b3,0x20eb=0xc000@0x20f9:0x00c0>25` the caller walks `0092 → 0094 → 00b0 → 00b2 → 00b6 → 00c0 → 00c1`, **the answerer's `0x00b0` hold breaks by itself** — `00b0 → 00b1 (DSR) → 00b2 → 00b3 → 00b6 → 00c0 → 00c2` — and after V.90A gives up at 26.0 s both ends fall back to page 8 and **reach `TrnProgress 0x00d0` with `CTS|DSR|DCD` on both ends, `DATASTATESpeed=0x1110`**. A same-session 60 s control with no pins has the caller at `0x0092` and the answerer at `0x00b0` for the whole call, so all of that is the stand-in's doing. ⚠ It is a stand-in: it makes the DSP see status it did not compute. **What is left is `0x00c1`, and it is a signal question, not a status one.** `0x00c0`'s exit needs `DM(0x10D9)` too, which is real and toggles once per 875 samples; forcing bit 15 lands on `0x00c1`, a 3,200-tick timeout state whose expiry raises `ratechange|flow_blocked` and asks for INFO — pinning `0x00c3`'s bit 12 as well changes nothing, because the abort is at `0x00c1`'s expiry and not at `0x00c3`. **And the exit that looked missing there is a *failure* branch, so do not force it.** `0x00c1`'s `test1` is `PM 0x33F8`, `DM(0x21E6) ≥ 1200`, and its `next[1]` — like `test2`'s `PM 0x3442` — is record `0x1938`, whose only exit is a 20-tick dwell to `0x1956`, a record with both handler slots set to the never-handler `PM 0x340A`: a terminal. Pinning `DM(0x21E6) = 0x04B0` at `0x00c1` aborts at three release times (25.3/25.6/25.9 s) for that reason, which also explains Session 250's ungated result. `0x00c1`'s success exit is the sequential one, `00c2 → 00c3`, and `0x00c3` wants bit 12 of `DM(0x20EB)` **and** `DM(0x10D9)`, which reads 1 for about two samples in every 875. **⚠ And satisfying that pair does not help, which is now a hard result rather than an instrument artefact.** `EICON_ANALOG_PIN_DM` grew a hard form, `ADDR=!VALUE`, that engages the core's own store hook while the gate matches instead of writing the word once a frame — needed because `0x00c2`'s record stores `0x0040` over `DM(0x20EB)` and `0x00c3` reads bit 12 of it a few instructions later, inside the same sample, where a soft pin never holds. Holding **both** `DM(0x20EB) = 0xD000` and `DM(0x10D9) = 1` against every store from `0x00c1` onward leaves the walk byte-identical: `00c1 → 00c3 → INFO`, with the cursor going `0x1938 → 0x1956` — the terminal — inside one sample. So `0x00c3`'s condition is not what routes the machine to the failure record — **`0x00c1`'s persisted `test2` does, and it is now pinned to one word.** `test2` is `PM 0x3442`: bit 9 of `DM(0x3EE6)` clear (it reads `0x0105`, so it is) and then `DM(0x2544) AND DM(0x254B)`, zero meaning branch — to `0x1938`, the terminal. Sampled across the call the pair is `0x0001/0x0001` for the whole page and `DM(0x254B)` drops to `0x0000` at the exact sample the attempt ends. Its writer is `PM 0x2CF1..0x2CF6`, and it is one comparison: `DM(0x254B) = (DM(0x2478) > 26)`, evaluated **once per call**, off the first word of the message the builder at `PM 0x2C8D` assembles at `I7 = 0x2478` from `DM(0x3F8B)`, `DM(0x3F0A/0x3F0B)`, `DM(0x103C)` and `DM(0x1AF8)`. It reads **20** against the threshold 26. **So the V.90A attempt does not fail on a missing status bit at all — it computes a number off the channel, finds it under a floor, and declines, which is the shape of a rate decision and is why the fallback that follows takes V.34 and completes.** Next: name `DM(0x2478)` — `PM 0x2C8D`'s inputs are all database words, so this is answerable without a live call — and then ask whether 20 is the loopback's channel or our own transmit. **Holding `DM(0x254B)` past that decline opens the rest of the ladder, and the whole of it is now walked.** With `0x254b=!0x0001@0x20f9:0x00c1` the page stops asking for INFO and the pairing climbs, one condition at a time, each one the same shape (`bit of DM(0x20EB)` plus `DM(0x10D9)`): `0x00c3` → `PM 0x349D` bit 12, `0x00c6` → `PM 0x349F` bit 10. Released at `>30` so the two ends stay in step, **the answerer reaches `TrnProgress 0x00d0` with `CTS｜DSR｜DCD` at 28.4 s on its own V.90D page** — not the V.34 fallback — and the caller walks `00c6 → 00ca → 00cc → 00cd`. **`0x00cd` is the last condition and it is a real one:** `PM 0x34A6` waits for `(DM(0x2104) & 0xFF) ≥ 0xD0`, the *peer's* state, and `DM(0x2104)` reads `0x0028` for the entire call — never written, so the caller never decodes the answerer's phase-4 control channel even though the answerer is sitting in data mode. Fabricating that word (`0x2104=!0x00d0@0x20f9:0x00cd`) does put **both ends at `0x00d0` on their own V.90 pages for the rest of the call** — which bounds what is left to exactly this one decode — but the caller then raises `CTS` only, against the answerer's `CTS｜DSR｜DCD`, so the fabrication is not equivalent to hearing the peer and **this is not a V.90A connection**. ⚠ **And `DM(0x2104)` is almost certainly not the peer's state — it is this page's own *inner* machine's.** No image in the analog109 set writes it absolutely (six absolute readers in V90.ANA, no writer), and the inner scheduler at `PM 0x33BB` copies its record's next-addresses from `DM(0x2105..0x2108)` and its tests from `DM(0x2109..0x210D)` — the same one-before-next[0] relationship the outer machine has between its state word `DM(0x20F9)` and `DM(0x20FA)`. So `0x00cd` waits for the **inner** machine to reach `0xD0`, and the inner machine — cursor `DM(0x2127)`, the `iptr` the trace already prints — is what to walk next. **⚠⚠ Walked, and it overturns this entire row's premise: the status vocabulary is not host-supplied, it is the inner machine's, written out of the same table.** `PM 0x33D2` is `PM 0x33DD` with the shifts moved — index is `A & 0xFF` instead of `A >> 8`, the value's low byte `B & 0xFF` instead of `B >> 8`, both taking the same `C & 0xFF00` — so **every three-word entry carries one assignment for each machine and the table is two programs**. `tools/record_table_decode.py --inner` reads the second one: 50 records, states `0x0000, 0x0010, 0x0020, 0x0028, 0x002c, …`, and the addresses it lists are exactly the `iptr` values the live trace prints. Its records write the words the outer machine waits on — **`0x1752` (inner state `0x5a`) sets index 6 to `0x0800`, which is bit 11 of `DM(0x20EF)`, `0x0092`'s bit**, and `0x18d2` (inner state `0xD0`) sets it to `0x1312`, carrying `0x00b3`'s bit 12. So "no image in the analog109 set writes the word" was true and irrelevant: the writer is a *record*, and the outer decode could not see it. Every pin in this row is therefore a stand-in for the inner machine not running, not for a missing host. **And why it does not run is a detector question.** The inner machine sits on record `0x16b6`, state `0x0028`, for the whole call — `DM(0x2104)` reads `0x0028` from the first sample to the last — and that record's only condition is index 36 = handler `0x0A` = `PM 0x3470`: `DM(0x10F3) XOR 1`, advance when the flag is set, and clear it. `DM(0x10F3)` is the tone/event flag `--trace-v90a-state` already prints as `event`. **And the last hop closes the chain, onto a mechanism this project has already met.** Tracked live on an unpinned control, the inner machine walks `0x0000 → 0x0010 → 0x0020` and stops there, on record `0x16a4`, at the same moment the outer machine enters its `0x0092` park — `event = 1`, threshold `700`. `0x16a4`'s primary is a dwell countdown (`PM 0x33EE`, on `DM(0x2103)`, 40 ticks) and its `test0` is the event flag `PM 0x3470` with **`next0` pointing at `0x16a4` — itself**. So the inner machine is in a **detector-driven self-loop that reloads its own dwell**, which is the *same* mechanism, threshold and shape as the V.34 answerer's correlator-latch self-loop on state `0x0090` in the row below — including the number 700. The complete chain is now: detector latches too often → inner state `0x0020` re-enters itself → inner records `0x1752`/`0x18d2` never run → `DM(0x20EF)` bit 11 never set → outer parks at `0x0092`. **Both open modem blockers in this file are one question: why a 700-threshold detector keeps firing on this rig — and it is literally one routine.** A write watch names the producer: `DM(0x10F3)` is set by `PM 0x0D04`, **43,202 times against 1,597 clears**, and `PM 0x0CE8..0x0D06` is the *same* 6-tap correlation as V.34's `PM 0x0E31` — `MY0 = $1554` (1/6 in Q15), `CNTR = 6`, `AR = ABS MR1`, `IF GT AR = 0 + 1` — over a 6-word circular buffer at `DM(0x2201)`/`L0 = 6` fed from a complex rotation, differing only in reading its threshold from `DM(0x20F7)` rather than `DM(0x2145)`. Both read 700. ⚠ Receive level is **not** the explanation on its own: `EICON_ANALOG_RX_GAIN_DB=-12.04` (the ×4 the CX needed) and `-24` both leave the inner machine on `0x0020` unchanged, while `-80` is a working positive control — the caller then never reaches the page at all. **Instrumented, and the level negatives now have a mechanism.** Exec-watching `PM 0x0D01` over 4,000 evaluations: threshold 700 on 3,992 of them, `ABS MR1` **2,329–5,736, median 4,537 — 6.5× the threshold, latching 100% of the time**, and the median moves by 16 counts across **24 dB** of line attenuation (4,537 → 4,553 → 4,553). The input is therefore not the line, and `PM 0x0D07` says why: it is a **normaliser**, a loop that halves `MX0`/`MX1` by `0x4000` until `ABS MX0 < 0x051E`, so everything downstream of it is level-independent by construction. That kills receive gain as an explanation for this detector on both pages, permanently. **What is left is a phase question:** a 6-tap boxcar (`MY0 = $1554`) over a *rotated* signal averages toward zero when the rotation matches and stays at full scale when it does not, so a magnitude pinned near full scale after normalisation is the signature of a constant-phase — DC-like — input, which is the same complaint as the `0x00b3` row below. Measure the mixer, `PM 0x0CF0..0x0CF6` and the phase accumulator at `DM(0x2200)`/`DM(0x2201)`, not the level. **First look at it, so the next session does not start from zero:** the accumulator is *not* frozen — 8,641 distinct values after 13 s — but it alternates between roughly `0x7xxx` and `0xFxxx`, a **half-cycle step**, i.e. the mixer is running at about Nyquist. Six taps of a signal that flips sign every step ought to cancel to nearly nothing, and this one reads 4,537, so either the step is aliasing to no rotation at all or the taps are not the samples the boxcar assumes. `DM(0x2201)` sits at `0x0E60`/`0x0E63` on the frame boundary against `L0 = 6`, `B0 = 0x0E60`, which is three taps advanced per line sample and is worth confirming before anything else. **The taps themselves are the finding: 3,000 of them and not one sign change.** Exec-watching the store at `PM 0x0CF8` gives a strictly positive sequence, `981..13,981`, absolute mean **4,553** — which is the boxcar's own output, 4,537, so `MR1` is simply the mean of taps that never go negative and therefore *cannot* fall under 700. The delay line is not shared scratch, the trap this project has hit twice: an ownership survey of `DM(0x0E60..0x0E65)` attributes **23,970 of 24,000 writes to `PM 0x0CF8`** itself. And the producer is not a level: `PM 0x0D07..0x0D33` normalises the pair, then runs a **nine-iteration rotate against a PM table at `0x26A0`** — the shape of a CORDIC — so the reading to confirm is that the taps are an *angle*, in which case a steadily one-signed mean is a persistent frequency offset between the caller's demodulator and what it receives, and that, not a level, is what has to be explained. ⚠ `--analog-codec-rate 8000` cannot test the 9600/8000 mismatch as the cause: the caller then parks at V.8 `0x0001`, the ANSam envelope failure `docs/ansam_envelope_loss.md` documents, and never reaches the page at all. **The CORDIC reading is confirmed, so the taps are angles.** `PM 0x26A0` onward is nine (shift, angle) pairs — `(ffff, 1b02) (8000, 168e) (4000, 1000) (2000, 0972) (1000, 04fe) (0800, 0289) …`, halving shifts against shrinking angle increments — and the loop accumulates them into `AR` with `AR = AR ± AY0` on the sign of `SR1`, which is arctangent by definition. So the tap is a **phase**, its mean of 4,553 is a *constant* residual rotation of about 25° per tap, and the six-tap boxcar is a frequency-offset detector reading far outside its 700 band. ⚠ Two candidate causes for that offset are already closed: the 9600/8000 boundary is resampled properly (`RationalResampler`, windowed sinc 6:5, not linear), and what the caller actually receives during the park is broadband — `tools/modem_tone_probe.py` on `caller.rx.ulaw` at 12.6–16 s gives RMS 403 with the peak bin wandering 750–3600 Hz at 5–8 dB over mean, i.e. the answerer's data signal, not a training tone. **Answered, and it closes this row down to one sentence: the detector is measuring its own oscillator, because its input is DC.** The frequency word handed to `PM 0x0CE8` is the constant **`AR = 0x11C7` = 4,551** on every one of 600 calls — and the CORDIC tap mean is **4,553**. The residual rotation *equals the NCO increment*, which is what arctangent returns when the vector being rotated is constant: the mixer has nothing to cancel. The input pair confirms it directly — `MX0` at entry takes **five distinct values across 600 calls** (`0x0003..0x0005`), i.e. a DC-like vector, not a signal. That is why the taps never change sign, why the normaliser makes the magnitude level-independent, and why the boxcar sits at 6.5× a threshold it can never fall under. `DM(0x2200)` itself steps ~`0x8000` per call, essentially exactly Nyquist for its ~14.9 kHz call rate (mean gap 2,220 cycles on a 33 MHz part), which is a degenerate value rather than a carrier and follows from the same thing. **So both §2 modem blockers reduce to: the V.90A receive chain is delivering a constant to its demodulator.** That is the same complaint as the `0x00b3` row below, on the receive side, and it is where the next session starts — walk back from `MX0` at `PM 0x0CE8` to whatever fills it. **Walked one hop, and the loss is bracketed.** `MX0`/`MX1` are the outputs of two biquad sections at `PM 0x0CB2`, driven from `PM 0x3535..0x3541`, and the signal is healthy going in and gone coming out: at entry `MX0` has **RMS 908** (−2,877..1,535) against `MX0` at `PM 0x0CE8` of **3..5**. So roughly 200× is lost inside those two sections, and that — not the detector, not the level, not the state machines — is the V.90A caller's actual fault. ⚠⚠ **And a trap found the hard way: the on-disk overlay image is not what executes at these addresses.** `PM 0x3537` disassembles from `026b-v90.ana-apcm-overlay/pm.bin` as `I4 = $2448`, the second section's coefficient base, but the *live* op there is `0x3C0105` (`CNTR = 0x0010`) and `I4` reads `0x3520` at both call sites. Most of the page does match — `MY0 = $1554` at `PM 0x0D01` and `I4 = 0x26B2` in the CORDIC both agree with the file — so this is a patched range, not a bad extraction. **Any reading of `PM 0x3520..0x3541` must come from a runtime PM dump, and the "wrong coefficient pointer" story this looked like is withheld until it does.** ⚠ The dump was taken (`EICON_DUMP_PM=0x0ca0:0x3560:…` with `EICON_WATCH_OVERLAY=0x026b`) **and it does not settle it — it contradicts the executing core.** At page load `PM 0x3537` dumps as `3a4480`, `I4 = $2448`, identical to the file, while the exec watch at that same address reports `op=3c0105`, `CNTR = 0x0010`, `pmovlay=0`, with `I4 = 0x3520` at both biquad call sites. A PM write watch over `0x3535:0x353D` catches only 18 stores, all from the loader's block clear at `PM 0x064D` at cycle 10,525, so no firmware store explains the difference. **The most likely reading is that `adsp2181_pm()` returns the base bank the way `adsp2181_dm()` does (§3), so the dump is not what executes** — in which case the executing `op=` is the ground truth and the coefficient pointer really is left at `0x3520`, pointing into code, which would be the 200× loss. **Banking is ruled out as the explanation, and the instrument is the suspect.** `adsp2181_pm()` does return the base bank, but `pmovlay` reads **0** at the seam, and the core only consults `program_overlay[]` for `pmovlay` 1..2 — so base bank *is* what executes here. The core's own comment says why the two can still differ: "the pages are swapped into the same PM by download, not selected by PMOVLAY". `EICON_DUMP_PM` fires **the first time the overlay becomes resident**, which is the switch instant, so it can photograph an image the download has not finished writing — which would explain both the 3,330 differing words and the one that matters. `EICON_DUMP_PM` now takes a trailing `:<seconds>` to settle past residency — **and taking it does not resolve the contradiction, it sharpens it into a bounded emulator question.** Dumped 2 s after the page loads, `PM 0x3537` still reads `3a4480`, and the core's own decoder confirms that word is `I4 = 0x2448` (`case 0x3a` → group 2, `op & 15` = 0 → `I4`, immediate `0x2448`), so the disassembler is not at fault either. But the exec watch at `pc=0x3538` reports **`op=17539e`** — a `DO … UNTIL`, not the `1ccb2f` `CALL $0CB2` that both the file and the settled dump hold — at `pmovlay=0`, where `ROPCODE` reads the same `program[]` array `adsp2181_pm()` returns. **Three things that cannot all be true: the core's PM array, the core's own instruction fetch, and the `pmovlay` it reports.** That is the next task, it is small and self-contained, and it is the same class as Sessions 46 and 52. Until it is settled, *nothing* read out of `PM 0x35xx` counts — including this row's `I4 = 0x3520` reading, which is now withheld | 249, 250, **251** |
