...
Current to **Session 229**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 229 sessions in six volumes under `analysis/`, the record of *how*
...
| **DIL is a lottery** | open, but the producer/consumer seam is found. Callback group 0 is generated at PM `1982..19a6`; PM `3232..323b` makes its first target from `DM(3fcb)+DM(3f04)+5`. The 7802 value selects PM `0375`, entering the empty entry-6 helper and saturating `DM(2f4f)` at `8000`; AudioCodes selects returning block-copy PM `022b`. Forcing the target or its upstream DPCM value removes the runaway. Determine why `DM(3fcb)` differs and whether PM `0375` needs omitted initialization; current forces are diagnostic | 88–93, 105–107, 212, 214–215, **229** |
