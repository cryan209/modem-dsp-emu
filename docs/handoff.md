...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); root cause identified (115b), fix pending a contract question.** `AX1` is the ring index — write address is exactly `0x0062 + AX1` — and it starts at `0xffff` and **steps by 2 forever**. The reset at PM `0x1938` (`DM(I1,M0) = AY0`) **executes zero times in 40 passes**: `0x1937` is an *equality* test, and the operand at `0x1936` steps `0002 0004 0006 …` (always even) against `AY1` = descriptor word 1 = `0x2863` (**odd**). Even stepping by 2 never equals odd, so the bound is unreachable and the pointer marches through low DM. Two one-line remedies, predicting the same A/B but different ADDSP contracts: publish `0xFFFE` as `BULK_DESCRIPTOR_LOWER_LIMIT` (but `0xFFFF` is documented as the deliberate -1 sentinel), or treat the step-of-2 near/far pairing as the anomaly. **Resolve from the ADDSP guide, then gate with `--assert-dm-clean 0x0061:0x0241@0x0261`** (baseline 69: 64 legitimate ISR ring at `0x00C0..0x00FF`, 4 the runaway loop). Note `0x0061..0x0241` was never a safe bound — it contains that ISR ring and the dispatch table at `0x00A8..0x00B4` | Sessions 106–108, 110–111, 114k–l, 114z, 115, **115b** |
