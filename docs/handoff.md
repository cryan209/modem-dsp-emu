...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); the obvious fix is disproved (115).** 114k–l bounded the ring pointer to `0x0061..0x0241` and verified "zero writes" against one table. Both claims are true and beside the point: **`DM(0x00A8..0x00A9)` is inside that range**, and PM `0x1930`/`0x1934` writing there is what freezes V.34. **`0x0061..0x0241` is not a safe bound at all** — the sample ISR's live 64-word ring (`L4=0x0040`) is at `DM(0x00C0..0x00FF)`, and the read-database dispatch table at `0x00A8..0x00B4`, both inside it. **Not the length:** the descriptor at `DM(0x0000)` reads `[0]=2852 [1]=2863 ...`, so `AY1` is non-zero and `AY1-AX0 = 0x11` is a real length over ring pointers at `0x2852..0x28ac`. Open question is why `AX0` — the write pointer captured at PM `0x1925` before `0x1927`'s correction — escapes into low DM. **Gate for any fix: `--assert-dm-clean 0x0061:0x0241@0x0261`** (new). Current baseline 69 violations: 64 legitimate ISR ring, 4 the runaway loop | Sessions 106–108, 110–111, 114k–l, 114z, **115** |
