...
| **the native V90D bulk worker corrupts DM** | **REOPENED — the fix moved the corruption, it did not remove it.** 114k–l bounded the ring pointer to `0x0061..0x0241` and verified "zero writes" against one nominated table (`DM(0x064B..0x066A)`). Both claims are true and both are beside the point: **`DM(0x00A8..0x00A9)` is inside `0x0061..0x0241`**, and 114z catches PM `0x1930`/`0x1934` writing there and freezing the V.34 page. Any bound still marches through something — the pointer fails to wrap, so the repair is to make it wrap or to stop driving the firmware pair on this page. **Verification must assert zero writes anywhere in `0x0061..0x0241`**, not into one table | Sessions 106–108, 110–111, 114k–l, **114z** |
