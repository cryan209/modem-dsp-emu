...
Current to **Session 230**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 230 sessions in six volumes under `analysis/`, the record of *how*
...
|---|---|---|
| **the CX93001 requests V.34, not V.90** | selection localized to original VG224 2/3/8403 path; alternate lines select V.90 normally. 7802 remains 0/9 for CX `CONNECT`, all at Eicon DIL. Same-CX replay now locates the causal dispatch: AudioCodes generates group-0 callback `DM(0)=022b`, while 7802 generates `0375`; dispatching PM `0375` enters the empty DIL helper. The formula is `DM(0)=DM(3fcb)+DM(3f04)+5`: `021a+0c+5=022b` good, `0364+0c+5=0375` bad. `DM(3fcb)` is scaled INFO elapsed-time carryover from gated counter `DM(3fc9)`, not RTDelay or measured echo. Forcing the target or carryover removes the runaway, diagnostically. Decode the INFO counter's epoch/stop condition. `S202=0`, `+MR=0`, no endpoint | 190–195, 215–229, **230** |
...
| **DIL is a lottery** | open, but the producer/consumer seam is found. Callback group 0 is generated at PM `1982..19a6`; PM `3232..323b` makes its first target from `DM(3fcb)+DM(3f04)+5`. The 7802 value selects PM `0375`, entering the empty entry-6 helper and saturating `DM(2f4f)` at `8000`; AudioCodes selects returning block-copy PM `022b`. `DM(3fcb)` is page 14's fixed-point-scaled inheritance of INFO's gated elapsed counter `DM(3fc9)`, not `RTDelay` (`DM(3f87)`). Determine why the INFO timing histories differ and whether PM `0375` needs omitted initialization; current forces are diagnostic | 88–93, 105–107, 212, 214–215, 229, **230** |
