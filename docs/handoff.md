...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); reframed (115d): it is an allocation conflict, not an escaped pointer.** `AY0` at PM `0x190e` is the ring **base** and reads `0x0062`, then `0x01dc` — so the bulk delay line **belongs in low DM** and the `0x0061..0x0241` march is the buffer itself, not trespass. The far branch (`0x193a..`) **never executes** on V.34, so there is no far pair; 115c's start/end reading of `[0..3]` is withdrawn. Sitting inside the same region: RDB result pointers `0x009B..0x00A7`, the **dispatch table `0x00A8..0x00B4`** (114z's `0xee1c`), the **sample ISR ring `0x00C0..0x00FF`** (`L4=0x0040`), and a second ring base at `0x01DC`. A near seed of 49 couples = 98 words from `0x0062` reaches `0x00C4` — over the dispatch table and into the ISR ring; 129 couples from `0x01DC` reaches `0x02DE`, past the region. **Not reseeded**: the gap is ~35 couples by inference, but three consecutive sessions have had their structural reading overturned by measurement, and 114k–l is in the tree because someone shipped on inference. Next: `--watch-dm 0x009b,0x00a8,0x00c0,0x01dc` for true extents, then reseed and gate on a **scoped** assertion (`0x009B..0x00B4` + `0x00C0..0x00FF` from non-ISR PCs) — the whole-range assertion counts the delay line as a violation | Sessions 106–108, 110–111, 114k–l, 114z, 115–115d |
