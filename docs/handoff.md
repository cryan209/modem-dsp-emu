...
| **the V.34 page freezes** | **open; cause isolated to a wild arrival into PM `0x2e1b`.** During `0x0261` residency the page runs **59 PCs / 7.49 G instructions**: the loop `0x2e1b–0x2e22` (99.7%) plus one ISR run exactly once per sample — the 8 kHz clock is perfect and PM `0x0771` never runs. **Retracted (114r/114v/114w):** the tables are intact *and* both legitimate entries via PM `0x2e1a` complete correctly, hitting their terminators at iterations 4 and 5. The hang is a **third arrival 875,000 cycles later that never passes through the loop head**, so `AY0 = $00FF` is never executed: it runs with `AY0=0x5a82`, `MR0=0x8000`, `MR1=0x026c` (Q15 twiddle constants, not a byte mask), `ret=0x2726`, `psp=9`, `lsp=1`, `cntr=12`. `AF = AX0 AND 0x5a82` never equals `0x026c`, so it spins forever. **It also writes `DM(MR0 + AF)` = `0x8000+AF` → masked to DM `0x0000..0x00ff` on every one of 941 M iterations** — a live corruption source worth re-checking against 106–114l. Next: watch `0x2725`/`0x2726` with `--watch-exec 0x2e1b:2` for the caller and its dispatch value. **Do not quote rate figures from 114m–114o** | Sessions 76–79, 114b–p, **114u, 114x** |
