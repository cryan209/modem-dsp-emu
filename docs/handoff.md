...
| **the native V90D bulk worker corrupts DM** | **CAUSE CONFIRMED (115j) AND WORKED AROUND (115k), not yet fixed.** `EICON_V34_BULK_HOLD=1` RTSes PM `0x19c8` on `0x0261`; `EICON_V34_PORTABLE_BULK=1` additionally serves the guide's near/far ABI from `PortableBulkDelay` (no changes needed — it is page-agnostic). Both **eliminate the freeze**: distinct PCs 59 → ~7,470; PM `0x0771` 0 → ~700 k; the `0x2e1b` runaway (941 M iters) gone; `TrnProgress` `0x0064` → **`0x0090`**; `--assert-dm-clean` clean of `0x1930`/`0x1934` and `0x2e21`. **Neither connects** (`NO CARRIER`, page cycles), and the portable delay adds **nothing measurable over the bare hold** — same PC count, same `0x0090` ceiling — so **the bulk delay is not the remaining constraint**. Next: `--watch-dm 0x3f36:40` to check whether the published output pairs are read at all; and 3–4 calls per configuration before flipping the default (the current default freezes and corrupts low DM, so flipping is likely right, but not on one call) | Sessions 106–108, 110–111, 114k–l, 114z, 115–115k |
