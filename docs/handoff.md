...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); six hypotheses tested, chain unmoved.** **New in 115h:** the driver's `DSP_CAI_MODEM_EXTENDED_LEC` (0x80) shares `cai[21]` with the symbol-rate disables and **this project has always sent it as zero** — the branch naming that byte only runs when `s7`/`s10` are set, so it ships as padding. Now settable with `EICON_EXTENDED_LEC=1` (default unchanged). The card **accepts it and acts on it**: descriptor `[6]`/`[7]` move `0aab`/`02a2` → `ee60`/`eeaf`, and *only* those two words — positively identifying them as echo-canceller config. It does **not** fix the freeze (`0x0064`, 931 M loop iterations, 59 PCs, identical). Also kills 115g's "negative `[7]`" idea: V.34 now has negative `[6]`/`[7]` and still hangs. **Unmoved since 114z:** PM `0x1930`/`0x1934` overwrite `DM(0x00A8..0x00A9)` → PM `0x2722` reads `0xee1c` → `CALL (I7)` enters at `0x2e1c` not `0x2e1a` → `AY0 = $00FF` never runs → 99.7% of the call in that loop, 8 kHz ISR exact. Next: same A/B on V.90, and the symbol-rate disables in the same byte | Sessions 106–108, 110–111, 114k–l, 114z, 115–115h |
