...
Current to **Session 235**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 235 sessions in six volumes under `analysis/`, the record of *how*
...
|---|---|---|
| **the CX93001 requests V.34, not V.90** | selection localized to original VG224 2/3/8403 path; alternate lines select V.90 normally. 7802 is now 0/15 for CX `CONNECT`. `DM(3fcb)` is RTDelay at 8 kHz resolution. The runaway comes from signed detector byte `SE=+1` versus healthy `-95`. Empty-only repair and positive-candidate rejection both remove the local runaway offline; each is 0/3 live. The reject A/B's third call reset `00b3 -> 0001 -> 0000`. PM `3d00` produces a 20-word filter ring, `2b4b` drains it, and `313f` stages the 36-word aligned ring. Compare that detector state; consequence guards cannot create the missing DIL response. `S202=0`, `+MR=0`, no endpoint | 190–195, 215–233, **234** |
...
| **DIL is a lottery** | open, with the exact arithmetic input found. PM `0388` loads a filter result's low byte into signed `SE`; PM `055d` computes `0-SE`. DATA: `-95 -> +95`; stall: `+1 -> -1`. Two guards eliminate the runaway but fail live. The fitted 2185N revealed a real emulator defect: Eicon writes SPORT0 control `0x4000/0x4035`, enabling BIASRND, while the core always rounded ties to even. BIASRND is now modeled and tested, but both canonical stalls remain stalled and the healthy replay remains healthy. Continue the 2185N SPORT/control-register and PM `3d00..3d22` filter audit | 88–93, 105–107, 212, 214–215, 229–234, **235** |
