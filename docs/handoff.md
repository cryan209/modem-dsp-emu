...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); best lead is now the descriptor base (115f).** An ownership survey of `0x0061..0x0241` (new `--assert-dm-clean LO:HI:BUDGET`) returns **one contiguous run, 481 words, owners `36fc,3738`** — **no boundaries at all**, so the region is not partitioned and there is no gap to "reseed into"; the dispatch table and the delay ring are two consumers of the same words. `publish_bulk_lower_limit()` reads the descriptor base from **`DM(0x32F7)`, which is `0x0000`** — an unpublished pointer. That explains everything: word 5 lands at `DM(0x0005)`, ring bases resolve to `0x0062`/`0x01DC` as offsets from zero, and the delay line is written across low DM including the dispatch table PM `0x2722` reads back. **The defect is probably the base, not a length.** Next: read `DM(0x32F7)` on a **V.90** call (same worker, page trains to `0x00c4`) — non-zero there is decisive — and grep the MIPS side for any write to `0x32F7`. Confirmed and stable: rings in low DM, `AX1` steps by 2 per the guide's (X,Y) couples, reset at PM `0x1938` never fires, dispatch table at `0x00A8` real and overwritten by PM `0x1930`/`0x1934` | Sessions 106–108, 110–111, 114k–l, 114z, 115–115f |
