...
| **the V.34 page freezes** | **still open. It is a hang, not a slow clock** — 114m's "200x slow" and 114o's "8x slow" are that hang averaged over page residency, and the kernel per-sample dispatch stops dead (`DM(0x3FB3)` is never rewritten; all 568 V.34-era dispatches are correct). Beyond that, **treat 114q–114s as retracted**: they conflated PM `0x2e24`'s block unpacker (loop `0x2e25..0x2e2e`) with the separate loop at PM `0x2e1b`, and compared counts across runs that watched different addresses. The `from=` histogram shows `0x2e1b` **terminates** — 6.2M iterations over 1.56M entries, ~4 each — so the pathology is that something calls it 1.5 million times, not that it is stuck, and block `0x1afa` is **not** shown to be corrupt. Ruled out: DM banking (loader and unpacker share the base bank). Next: enable `TRACK_HOTSPOTS`/`pcbucket` in `adsp2181_core.c` and dump per-PC execution counts for a V.34 and a V.90 call — per-execution logging is what produced the error | Sessions 76–79, 114b–p, **114t** |
