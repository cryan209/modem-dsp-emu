...
| **the V.34 page freezes** | **still open, but now localised.** It is a **hang, not a slow clock**. The kernel per-sample dispatch runs at rate for exactly 568 samples after the V.34 page installs `DM(0x3FB3)=0x19d5`, then stops dead and the CPU spends **98.6% of the call's cycles** in the loop at PM `0x2e1b–0x2e22`. The callback word is never rewritten (two writes all call), so there is no dispatch loss. Sessions 114m–114o's "200x slow" / "8x slow" rates are that hang averaged over page residency. The loop is **ordinary machinery, not a V.34 routine**: in the control call it runs 4,576 times across two V.90 DPCM eras and two INFO eras without ever interrupting the sample clock, then 7,786,658 times once V.34 is resident, after which no sample is ever dispatched again. The fault is its exit test at PM `0x2e21` (`AR = MR1 XOR AF` / `IF NE JUMP $2E1B`) never being satisfied under V.34's setup, so `I4` runs off its table and sweeps data memory forever. The loop is PM `0x2e24`, the **block loader's field unpacker** (`ret=0x2ddb`), entered once per block with `I4` = the block address and `MR0 = 0x2137`. Eleven entries per call; the eleventh is block **`0x1afa`** and it never returns. So the whole chain from 114m's "200x slow" ends at one instruction pair — the exit test at PM `0x2e21`/`0x2e22`, `AR = MR1 XOR AF` / `IF NE JUMP $2E1B`, `MR1 = 0x0019`. **Unresolved:** the static record for `0x1afa` carries that terminator at the same offset as blocks that return fine, so either the record differs at runtime or the stride does. Next: add `m5` to the `[EXEC]` format and re-run — the loop indexes `DM(I4,M5)` and `m5` is not currently logged | Sessions 76–79, 114b–o, **114p–r** |
