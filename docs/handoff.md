...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); best current account (115c): the seeded bulk lengths do not fit the rings.** `AX1` is the ring index (write address = `0x0062 + AX1`), starts at `0xffff`, steps by **2** forever, and the reset at PM `0x1938` **never executes** (equality test at `0x1937`: even operand vs odd `AY1`). The ADDSP guide (`0xDC`/`0xDD`) defines Nearbulklength/BulkLength as counts of **(X,Y) couples**, so the 2-word step is **by design** — that candidate remedy is ruled out. The guide does **not** document word 5 for this structure, so the `0xFFFF` "-1 sentinel" is this repo's inference. **The lead:** descriptor start/end pairs give rings of **17 words** (`2852..2863`) and **54 words** (`2876..28ac`), while `_service_bulk_lengths()` seeds **near=49 far=129 couples** = 98 and 258 words — ~5x oversize, and `BULK_SEED_CEILING = 0x0B00` (2816) cannot be right for a 17-word buffer. A pointer told to wrap after 98 words in a 17-word ring walks out of it; parity is then a symptom, not the cause. Fix the seed, not the parity, and gate with **`--assert-dm-clean 0x0061:0x0241@0x0261`** (baseline 69: 64 legitimate ISR ring at `0x00C0..0x00FF`, 4 the runaway loop) | Sessions 106–108, 110–111, 114k–l, 114z, 115–115c |
