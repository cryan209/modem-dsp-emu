...
| **the V.34 page freezes** | **open, narrowed to one call site.** For all of `0x0261`'s residency the page runs **59 PCs / 7.49 G instructions**: the loop at PM `0x2e1b–0x2e22` (99.7%, entered once, never left) plus one interrupt handler run **exactly once per sample** — the 8 kHz clock is perfect and PM `0x0771` never runs at all. PM `0x2e1a` is entered **twice per call**, 182 cycles apart: `I4=0x202e, MR1=0x0019` (returns) and **`I4=0x1ea2, MR1=0x0024` from PM `0x2d92` (never returns)**. Both tables carry their terminator in the downloaded image — at iterations 4 and 5 — so the table at `0x1ea2..0x1eb1` is not what the image says by the time it is scanned. The loop cannot corrupt its own source (it writes `0x2137 + byte`). Next: `--watch-dm 0x1eb1`, the single word that should stop the scan — reads give the live value, any `dm w` names the writer. **Do not quote any rate figure from 114m–114o**: they measured how often a hung machine got interrupted | Sessions 76–79, 114b–p, **114u–v** |
