...
**So the sample does arrive, and something zeroes `RXSAMPLE` once per frame; the V.90A page reads it after the zeroing and gets its constant.** That is the whole blocker, measured rather than inferred, and it is the first statement in this row that is upstream of every other one. Next: whether `PM 0x1733` is a correct double-buffer flip whose other half the page should be reading, or whether this harness runs the clear after the fill instead of before — and that is a question about frame ordering on the analog kernel-dispatch backend, so it wants a control on a page that works before anything is changed | 249, 250, **251** |
