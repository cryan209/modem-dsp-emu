...
| **V.32: the answering side's transmit is 6 dB worse, and it follows `GEN_SETUP1` bit 3** | **the live blocker, now a one-variable A/B that runs in 90 s with no hardware.** Swapping only the modulation roles — SIP roles, ports and processes unchanged — moves the deficit with the role: baseline 22 dB / 16 dB, swapped 16 dB / 22 dB, and in both runs the end holding `0x0484` (answer) is the one transmitting at 16. So it is what bit 3 selects in the transmit chain, not the instance, the socket or the page.|
| *(evidence for the row above)* | **reproducible in 90 s with no hardware.** Both ends forced to page 2 with `EICON_FORCE_DM=0x3fc4=0x2000@0x025f`, `eicon_loopback.py`, identical firmware, no network: the answerer's receiver reads `SNRatio` 22 dB on the caller's transmit, the caller's receiver reads **16 dB** on the answerer's. Only the role differs. Live, slmodemd scores our transmit at 8 dB against its 13 dB threshold and retrains every ~10.5 s, and we always answer. Not the width, not LAPM, not level, not the G.711 path: with `--tx-prbs`, and with the transmit mailbox never written at all, the retrain is identical, and our own receiver reports a pristine line the other way (`RXLevel` −10 dBm, MAE 0, `PeakPhasErr` 0, `FreqOffset` 0, `TimOffset` ±1, SNR 30–39.5 dB) | 204 |
