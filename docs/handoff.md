...
| **the V.34 page freezes** | **open, and now measured directly.** For the whole of `0x0261`'s residency the page executes **59 distinct PCs and 7.49 billion instructions**, and they are two things: the loop at PM `0x2e1b–0x2e22`, 933,978,490 iterations, **99.7%** of everything — entered once before residency and never left, since the loop head `0x2e1a` never executes — and one interrupt handler (`0x0014` → `0x0072..0x00c9`) run **374,080** times against 374,080 samples of residency, i.e. **the 8 kHz clock is exact**. PM `0x0771`, the kernel per-sample dispatch, never runs at all. So: a genuine infinite loop in the foreground, with a perfect sample interrupt returning into it (`RTI` at `0x00c9` — that was 114t's unexplained `from=00c9`). Retracted: 114r/114s blamed the block unpacker at `0x2e24`, which is a separate loop and never runs here; 114t then over-corrected to "the loop terminates", which was read off the V.90 control where it is healthy. **Every rate figure in 114m–114o measured how often a hung machine got interrupted — do not quote them.** Next: `--pc-histogram-from 0x0260 --watch-exec 0x2e1a` to bracket the loop entry, which happens before residency | Sessions 76–79, 114b–p, **114u** |
