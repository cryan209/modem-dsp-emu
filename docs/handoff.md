...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); five hypotheses dead, chain still solid.** `DM(0x32F7)` is **zero on V.90 too**, and V.90 trains to `0x00b0` — so a zero descriptor base is normal and 115f is dead, as are the corrupt record (114r/v/w), parity-as-cause (115b), oversized seed (115c) and allocation conflict (115d/e). Also: **no overlay writes `DM(0x32F7)`** (the `026e` "write" was a mis-indexed scan; `pm.bin` is 3 bytes per word). **Unmoved since 114z:** PM `0x1930`/`0x1934` overwrite `DM(0x00A8..0x00A9)`, PM `0x2722` reads `0xee1c`, `CALL (I7)` enters the scan loop at `0x2e1c` not `0x2e1a`, `AY0 = $00FF` never runs, loop spins 99.7% of the call, 8 kHz ISR exact. **Best remaining lead — diff the pages, don't theorise:** descriptors differ only in `[6]`/`[7]`, `0x0aab`/`0x02a2` (V.34) vs `0xffa5`/`0x094c` (V.90); `0xffa5` is *negative* signed, and `[7]` is what PM `0x1904` loads into `SR1` and PM `0x192f`/`0x1930` add to the index. Next: trace PM `0x1900..0x1935` on a V.90 call and diff against the captured V.34 trace | Sessions 106–108, 110–111, 114k–l, 114z, 115–115g |
