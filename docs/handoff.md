...
| **in state `0x00b3` we transmit a DC level, not a signal** | open, and separate from the companding. `DM(0x3FB4)` holds the generic pointer `0x3764` that PM 0x19ee re-primes whenever PM 0x1a1e's serializer did not run, and this path emits it as a sample: `14180`, a `-7 dBFS` DC level, on every one of the 15,875 archived page-14 frames in `0x00b3` and half of `0x00c2`. That is the state §7.10's six non-LAPM calls stopped in. The generic path would *dereference* it; page 14 deliberately does not (the comment at `frame_fast`), and which of the two the firmware intends in this state is the open question. Counted in the end-of-call census, behaviour unchanged. **Settled in 267: not the pointer.** `DM(0x3764)` takes no writes at all while page 14 is resident, `PM 0x19ee` is a context *restore* of the page's saved copy at `DM(0x3607)` rather than a prime, and the sample leaves through `PM 0x1a1e` one word per sample -- so taking the value is right. The zeros came from the mapping-frame block `DM(0x3fa7..0x3fac)`, refilled once per six samples and zeroed every sample by the kernel frame path at `PM 0x06c6`: 16.7% nonzero, held to 100% by `EICON_V90D_TX_BLOCK_HOLD` | **245, 267** |
...
- **⚠⚠ Residency is not the same thing as the page having something to look
  at.** `EICON_WATCH_OVERLAY` holds arming until the overlay is resident, which
  fixed the trap below it — and on its own it is still not enough. On the V.90A
  caller the page becomes resident at 9.36 s and the answerer transmits nothing
  until 14.6 s, so a budgeted watch armed on residency spends its entire budget
  on 5.25 s of silence and reports the received sample as a constant. Whole-
  residency summaries (`adsp.csv`, `EICON_ANALOG_DM_CSV`) have the same problem
  from the other end: the dead window dominates the mean. **Three sessions and
  six commits of "RXSAMPLE is dead" came out of this one window**, and all of
  them dissolved the moment the same watch was armed later. Pass
  `EICON_WATCH_AFTER=<seconds>` with the overlay gate, and before believing any
  null result about a received signal, check `caller.rx.wav`'s RMS in the window
  the measurement actually covered. 249–251, **252**.
...
1. **DM write watches** (`adsp2181_watch_dm_writes`). The line carries `ppc`,
   ...
   **Arm them with both gates.** `EICON_WATCH_OVERLAY=<id>` holds arming until
   the page under test is resident and `EICON_WATCH_AFTER=<seconds>` holds it
   until the call has also run that long; the second is not optional on any
   question about a received signal, for the reason in §4. Both gates apply to
   `--assert-dm-clean`'s range survey as well. `EICON_DUMP_PM` and
   `EICON_DUMP_DM` take the same delay as a trailing `:<seconds>` field, and a
   PM dump compared word-for-word against the overlay files is how "which
   library is actually resident" is answered — the low-PM shell library
   (`PM 0x1661..0x17D9`) differs between overlays, so a PC in it means nothing
   until the resident image is named.
