...
Current to **Session 233**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 233 sessions in six volumes under `analysis/`, the record of *how*
...
|---|---|---|
| **the CX93001 requests V.34, not V.90** | selection localized to original VG224 2/3/8403 path; alternate lines select V.90 normally. 7802 is now 0/12 for CX `CONNECT`. `DM(3fcb)` is RTDelay at 8 kHz resolution. The DIL runaway comes from signed ring byte `SE=+1` versus a healthy `-95`. An empty-only guard preserves valid arithmetic, leaves a healthy replay at `0x00d0`, and advances both stalled replays past the permanent loop. Live, however, 3/3 patched 7802 calls still stopped at `0x00b3` and produced only mark fill. Removing the runaway is not a valid DIL response; trace candidate production/validation upstream. `S202=0`, `+MR=0`, no endpoint | 190–195, 215–232, **233** |
...
| **DIL is a lottery** | open, with the exact arithmetic input found. RTDelay chooses a callback and delay-aligned ring pointer. PM `0388` loads the ring word's low byte into signed 8-bit `SE`; PM `055d` computes `0-SE`. DATA: `-95 -> +95`; stall: `+1 -> -1`, then saturation. An empty-only guard maps only `(AY0==0 && result<0)` to `+1`; it preserves a healthy `0x00d0` replay and moves canonical stalls to `0x00c2`, but 0/3 live CX calls connected. Do not ship the guard: identify why PM `313f..3141`/`0360..0388` accepts a positive candidate | 88–93, 105–107, 212, 214–215, 229–232, **233** |
