...
Current to **Session 236**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 236 sessions in six volumes under `analysis/`, the record of *how*
...
|---|---|---|
| **the CX93001 requests V.34, not V.90** | selection localized to original VG224 2/3/8403 path; alternate lines select V.90 normally. 7802 is now 0/18 for CX `CONNECT`. The runaway comes from signed detector byte `SE=+1` versus healthy `-95`. Empty-only repair and positive-candidate rejection remove it offline but each failed 0/3 live. The fitted 2185N exposed missing BIASRND emulation; implementing it preserves healthy/stalled replay classification and is also 0/3 live, all at `00b3`. PM `3d00` produces the filter ring. Compare its inputs and audit the remaining 2185N SPORT/control-register boundary. `S202=0`, `+MR=0`, no endpoint | 190–195, 215–235, **236** |
...
| **DIL is a lottery** | open. PM `0388` loads a filter result's low byte into signed `SE`; PM `055d` computes `0-SE`. DATA: `-95 -> +95`; stall: `+1 -> -1`. Two consequence guards fail live. The fitted 2185N revealed missing BIASRND emulation: Eicon writes SPORT0 control `0x4000/0x4035`, while the core always rounded ties to even. BIASRND is now modeled/tested, but canonical replays are unchanged and 3/3 live calls still stopped at `00b3`. Continue the 2185N SPORT/control-register and PM `3d00..3d22` filter audit | 88–93, 105–107, 212, 214–215, 229–235, **236** |
