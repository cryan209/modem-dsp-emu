...
| **the V.34 page freezes** | **open; narrowed to one loop, and the data is proven innocent.** During `0x0261` residency the page runs **59 PCs / 7.49 G instructions**: the loop at PM `0x2e1b–0x2e22` (99.7%, entered once, never left) plus one ISR run **exactly once per sample** — the 8 kHz clock is perfect, PM `0x0771` never runs. The hang is the second of two entries to PM `0x2e1a`: `I4=0x1ea2, MR1=0x0024`, called from PM `0x2d92`. **`DM(0x1eb1)` holds `0x0d24` — the terminator — is read 170,802 times and is never written.** So the table is intact and the *comparison* fails, not the data; the corruption theory has now been wrong three times (114r, 114v). Leading hypothesis: the walk loses its three-word alignment, since only the first word of each triple sets `AF`. The ISR saves `I4`/`L4`/`M5` to `DM(0x2E4A..0x2E4C)` — **three plain words, not a stack** — so a nested interrupt would restore the wrong `I4`. Emulator bank/`MSTAT` save-restore was checked in source and is correct. Next: a **bounded** `--watch-exec` (log first N hits) to read `AF`/`MR1`/`AY0`/`I4` at PM `0x2e21` for ten iterations. **Do not quote rate figures from 114m–114o** | Sessions 76–79, 114b–p, **114u–w** |
