...
| **the native V90D bulk worker corrupts DM** | **REOPENED (114z); the delay-line-sizing family is now ruled out in kind (115h–i).** Two never-before-set CAI controls in `cai[21]` — `EXTENDED_LEC` (0x80) and the symbol-rate disables (0x01..0x20), both shipped as padding for the life of the project — are now settable (`EICON_EXTENDED_LEC=1`, `EICON_DISABLE_SYMBOLS=...`; defaults unchanged). **The card accepts both and both resize the delay line** — descriptor `[6]`/`[7]` go `0aab`/`02a2` → `ee60`/`eeaf` (LEC) → `0ae8`/`021a` (2400-only), and nothing else moves. **Neither perturbs the failure at all**: `0x0064`, ~940 M loop iterations, 59 PCs, assertion identical, seven consecutive calls. So the freeze is insensitive to delay-line sizing, and 114k–l's bound, 115c's oversized seed and 115d's allocation conflict are wrong *in kind*, not just individually. PM `0x1930`/`0x1934` hit `DM(0x00A8..0x00A9)` under every configuration. **Next: 114z's other remedy — hold the worker entirely** (cf. `EICON_V90D_BULK_ADAPTER=0`); if V.34 still freezes with it held, the worker is not the cause | Sessions 106–108, 110–111, 114k–l, 114z, 115–115i |
