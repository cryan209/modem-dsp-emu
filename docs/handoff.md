...
| **the native V90D bulk worker corrupts DM** | **CAUSE CONFIRMED BY INTERVENTION (115j).** `EICON_V34_BULK_HOLD=1` RTSes PM `0x19c8` (`JUMP $1900`, identical on both pages) on `0x0261`. **The freeze disappears:** distinct PCs 59 → **7,464**; PM `0x0771` (kernel per-sample dispatch) 0 → **701,482**; the `0x2e1b..0x2e22` runaway (931 M iterations, 99.7%) gone from the hot set; highest `TrnProgress` `0x0064` → **`0x0090`**; no writes from `0x1930`/`0x1934`; state machine advancing at 51.7 s of a 52 s call instead of stopping at 5.3 s. So 114z's chain is causal and complete, and 115i's negative results are explained — configuration was never the variable. **Still a diagnostic, not a fix:** call ends `NO CARRIER`, `0x0261` loads 14x and cycles rather than trains, because the hold removes the far-echo bulk delay V.34 genuinely needs. **Repair path:** mirror `V90D_PORTABLE_BULK` — hold the native worker (now known safe) and serve the guide's near/far delay-line ABI (`DM(0x3FBC)`/`DM(0x3FBD)`, `BulkInputX/Y`, `BulkOutpuTX/Y`) from `PortableBulkDelay`, gated by `--assert-dm-clean` and the PC histogram | Sessions 106–108, 110–111, 114k–l, 114z, 115–115j |
