...
| **the V.34 page freezes** | **open; root cause one step away.** PM `0x2719–0x272b` is a **read-database publisher**: 13 routine addresses at `DM(0x00A8)`, called via `CALL (I7)` at PM `0x2725`, each result stored through `DM(0x009B)` into DB words `0x3F78..0x3F84`. **Pass 1 calls all 13 correctly** (matches the image exactly); on **pass 2 slot 0 reads `0x11e4` and slot 1 reads `0xee1c`**, which masks to PM **`0x2e1c`** — landing two instructions inside the scan loop, so `AY0 = $00FF` never runs and the loop spins on the publisher's leftover registers (`AY0=0x5a82`, `MR0=0x8000`, `MR1=0x026c`) for 941 M iterations, 99.7% of the call. `0xee1c` has bits above the 14-bit PM range, so it is **data written over the table**, not a mangled address. The hung loop then stores to `DM(MR0+AF)` = `0x8000+AF` → masked **DM `0x0000..0x00FF`**, which *contains that same table* (`0x00A8..0x00B4`) — self-sustaining, and possibly the real source of the DM corruption blamed on the bulk worker since 106. Next: **`--watch-dm 0x00a8,0x00a9`** — the `dm w` line names the writer, and that writer is the fault. **Do not quote rate figures from 114m–114o** | Sessions 76–79, 114b–p, **114u, 114x–y** |
