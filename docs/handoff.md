...
Current to **Session 234**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 234 sessions in six volumes under `analysis/`, the record of *how*
...
| **DIL is a lottery** | open, with the exact arithmetic input found. RTDelay chooses a callback/aligned ring pointer; PM `0388` loads a filter result's low byte into signed `SE`; PM `055d` computes `0-SE`. DATA: `-95 -> +95`; stall: `+1 -> -1`. Two narrow guards eliminate the runaway but fail live: empty-only repair 0/3, positive-candidate reject 0/3. The latter can reset through secondary receive data. Trace first divergence in PM `3d00..3d2b` filter output and PM `0360..037b` candidate metrics. Do not ship either guard | 88–93, 105–107, 212, 214–215, 229–233, **234** |
