...
Current to **Session 232**. `eicon_adsp_firmware_analysis.md` is the index to the
running log — 232 sessions in six volumes under `analysis/`, the record of *how*
...
|---|---|---|
| **the CX93001 requests V.34, not V.90** | selection localized to original VG224 2/3/8403 path; alternate lines select V.90 normally. 7802 remains 0/9 for CX `CONNECT`, all at Eicon DIL. `DM(3fcb)` is RTDelay at 8 kHz resolution: INFO state 32→36 measures 67.5 ms AudioCodes and 108.75 ms 7802. Session 232 moves beyond delay: a successful Courier with the same published RTDelay 11 loads ring word `0x0aa1` into 8-bit `SE` as -95; 7802 loads `0xf201` as +1. PM `055d` computes `0-SE`, yielding +95 versus -1 and the runaway. Find the missing sign/validity gate or why the delay-aligned DIL sample is positive. `S202=0`, `+MR=0`, no endpoint | 190–195, 215–231, **232** |
...
| **DIL is a lottery** | open, with the exact arithmetic input found. RTDelay chooses a callback and delay-aligned ring pointer. PM `0388` loads the ring word's low byte into signed 8-bit `SE`; PM `055d` computes `0-SE`. DATA example: `0x0aa1 -> SE=-95 -> +95` countdown. 7802: `0xf201 -> SE=+1 -> -1`, then saturation at `8000`. The source word is normally written by PM `3141`; forcing callback PM `0369` does not change the bad sign. Find the absent sign/validity gate or detector condition. The previous “DM(2f4f) uninitialized” wording is superseded: zero is common, signed `SE` diverges | 88–93, 105–107, 212, 214–215, 229–231, **232** |
